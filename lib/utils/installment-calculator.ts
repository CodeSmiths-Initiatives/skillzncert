...
/**
 * Helper function to get plan from either ONLINE or SIWES plans
 * Detects SIWES plans by "-siwes" suffix in planId
 */
function getPlanByIdAndType(planId: string): PaymentPlanConfig | null {
  const isSiwes = planId.includes("-siwes");
  const plans = isSiwes ? SIWES_PAYMENT_PLANS : ONLINE_PAYMENT_PLANS;

  // Extract base plan ID (remove -siwes suffix if present)
  const basePlanId = planId.replace("-siwes", "").toLowerCase();

  return plans[basePlanId] || null;
}
...
export function calculatePaymentSchedule(
  ...
  const plan = getPlanByIdAndType(planId);
...
export function calculateInstallmentDueDate(
  ...
  const dueDate = new Date(startDate);
  const monthsToAdd =
    (installmentNumber - 1) * plan.installments.intervalMonths;
