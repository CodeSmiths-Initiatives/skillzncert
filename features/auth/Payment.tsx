...
const TABS = [
  ...
  { label: "Post Secondary Payment Plan", plans: POST_SECONDARY_PAYMENT_PLANS },
];

export default function PaymentPage({ userEmail }: Props) {
  const [activeTab, setActiveTab] = useState(0);
  const [selectedPlan, setSelectedPlan] = useState<Plan>(
    Object.values(TABS[0].plans)[0],
  );
  const [isLoading, setIsLoading] = useState(false);
  const [initializePayment, setInitializePayment] = useState<any>(null);
  const router = useRouter();

  // Update selected plan when tab changes
  useEffect(() => {
    const firstPlanInTab = Object.values(TABS[activeTab].plans)[0];
    setSelectedPlan(firstPlanInTab);
  }, [activeTab]);
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-gradient-to-br from-[#0b3c42] via-[#1b6b73] to-[#51A8B1] px-4 sm:px-6">
      ...
      >
          ...
          {TABS.map((tab, i) => (
            <button
              ...
              className={`px-5 py-2 rounded-md text-sm font-medium border transition-colors duration-200
								${
                  activeTab === i
                    ? "bg-white text-[#0b3c42] border-white shadow-lg"
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {Object.values(TABS[activeTab].plans).map((plan) => {
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div>
              ...
              {selectedPlan.installments.count > 1 && (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Next:{" "}
                    {formatAmount(selectedPlan.installments.subsequentPayment)}{" "}
                    every {selectedPlan.installments.intervalMonths} months ·
                    Total: {formatAmount(selectedPlan.amount)}
                  </p>
                </motion.div>
              )}
              ...
              >
                Bank Charges:{" "}
                <span className="text-orange-600">{} × % of Paystack</span>
              </motion.p>
              <motion.p
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
              ...
              >
                {selectedPlan.installments.count > 1
                  ...
                  : "Total"}
                :{" "}
                <span className="text-[#0b3c42] text-base">
                  {" "}
                  {formatAmount(getPaymentAmount())} +{" "}
                  {formatAmount(200000)}{" "}
                </span>
